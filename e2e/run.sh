...
function pull_images(){
  ...
  if [[ -z $(docker images -q cuijx/postgres-operator:${operator_tag}) ]]
  then
    docker pull cuijx/postgres-operator:latest
  fi
  if [[ -z $(docker images -q cuijx/postgres-operator-e2e-tests:${operator_tag}) ]]
  then
    docker pull cuijx/postgres-operator-e2e-tests:latest
  fi

  operator_image=$(docker images --filter=reference="cuijx/postgres-operator" --format "{{.Repository}}:{{.Tag}}" | head -1)
  e2e_test_image=$(docker images --filter=reference="cuijx/postgres-operator-e2e-tests" --format "{{.Repository}}:{{.Tag}}" | head -1)
}
